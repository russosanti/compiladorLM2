...
public class NodoProc implements INodo{
	...
	public void generateCode() throws CodeException {
		...
		if(getParam().listSize()>0){
			...
			cg.codeDeident();
			cg.insertCode("RET "+2*getParam().listSize());SingleTabla.getInstance().setContexto(this.getId());
		}else{
			cg.codeDeident();
	...
	public void generateDecl() throws CodeException {
		ITree cg = ExecutionTree.getInstance();
		SingleTabla.getInstance().setContexto(this.getId());
		SingleTabla.getInstance().setContexto(this.getId());
		cg.insertCode(";Inicio Declaraciones "+this.getId());
		this.getDecl().generateCode();
		cg.insertCode(";Fin Declaraciones "+this.getId());
		cg.insertCode("");
	}
}
