...
public class TablaDeSimbolos implements iTablaDeSimbolos{
	 ...
	 * @throws TableException */
	public boolean declaredIDinContext(String contexto,String s) throws TableException{
		if(contexto == null || contexto.equalsIgnoreCase("global")){
			return this.existeIDGlobal(s);
		}else{
			if(this.tablasContexto.containsKey(contexto)){
	...
	/** Busca en el contexto
	 * @throws TableException */
	public boolean declaredIDinContext(String s) throws TableException{
		return this.declaredIDinContext(this.contexto, s);
	}
	/** Busca en Func 
	 * @throws TableException */
	...
	@Override
	public boolean checkParamDesconocidas(String id,ArrayList<Tipo> params) throws TypeException {
		DeclFuncProcDesconocidas func = this.tablaFuncProcDesconocidas.get(id);
		if(func==null){			
				throw new TypeException("La funcion "+id+" no se encuentra declarada");			
		}else{
			if(func.getTipoParams() != null && params != null){
				if(func.getTipoParams().size() == params.size()){
					Iterator<Tipo> it1 = func.getTipoParams().iterator();
					Iterator<Tipo> it2 = params.iterator();
					int i = 1;
					while(it1.hasNext()){
						if(it1.next() != it2.next()){
				...
				if(((func.getTipoParams()==null)&& (params == null || params.size()==0)) || ((params==null)&& (func.getTipoParams() == null || func.getTipoParams().size()==0))){
					return true;
				}else{
					throw new TypeException("Error en el pasaje de parametros de la funcion: "+id, "No se pasaron parametros o se pasaron parametros a una funcion que no tiene");
				}
	...
	public boolean prohibitedID(String s){
		if(s.equalsIgnoreCase("showC") || s.equalsIgnoreCase("showSTR") || s.equalsIgnoreCase("showNUM") || s.equalsIgnoreCase("read")){
			return true;
		}
		return false;
	}
