...
import analizador.lexico.tokens.OperadorBooleano;
...
public class EXPBOOL extends NoTerminal implements ReglaSemantica {
	...
	public boolean accionSemantica(Stack<LRApilable> prod) throws TypeException {
		if(prod != null && prod.size()==3){
			try{
				OperadorBooleano tok = (OperadorBooleano)prod.get(1);
				if(tok.getType()==TokenTypes.OPERADOR_BOOLEANO){
					return accionSemantica((EXP2)prod.get(2),tok,(EXP2)prod.get(0));
				}else{
					if(tok.getType()==TokenTypes.OPERADOR_BOOLEANO_E){
						return accionSemantica2((EXP2)prod.get(2),tok,(EXP2)prod.get(0));
					}else{
	...
	private boolean accionSemantica(EXP2 exp, OperadorBooleano op, EXP2 exp2) throws TypeException{
		if(exp.getTipo() != exp2.getTipo()){
			...
			this.arbol = new NodoExpBool(exp.getArbol(),TipoOperacion.tipoValueOf(op.getLexema()),exp2.getArbol());
			this.tipo = Tipo.BOOLEAN;
			return true;
		}
	}
	
	private boolean accionSemantica2(EXP2 exp, OperadorBooleano op, EXP2 exp2) throws TypeException{
		if(exp.getTipo() != Tipo.INTEGER || exp2.getTipo() != Tipo.INTEGER){
			...
			TipoOperacion o = TipoOperacion.tipoValueOf(op.getLexema());
			this.arbol = new NodoExpBool(exp.getArbol(),o,exp2.getArbol());
			this.tipo = Tipo.BOOLEAN;
			return true;
		}
	}
