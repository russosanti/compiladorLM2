...
public class SintacticAnalyzer {
	...
	public int compile(){
		if(this.la != null){ //si Lexico no es nulo x no inicializar
			this.tree = ExecutionTree.getInstance(this.inpath,this.filename);
	...
	private boolean DespRed(){
		boolean declFunc = false;
		boolean pasoParamCierre = false;
		boolean accept = true;
						...
						switch(act.getType()){
							...
							case SYNTHETISE: //desplazar
								...
								if(t.getType() != TokenTypes.EOF){
									...
									//Eso lo hago para cambiar el contexto de la tabla
									...
									if(declFunc && tokenAnt.getType() == TokenTypes.PARENTESIS_CIERRE){
										if(this.t.getType() == TokenTypes.DEF_TIPO){
											pasoParamCierre = true;
										}else{
											iTablaDeSimbolos tablaSimbolo = SingleTabla.getInstance();
											tablaSimbolo.setTipoPendiente(Tipo.VOID);
											declFunc = false;
										}
									}
									if(declFunc && pasoParamCierre && this.t.getType() == TokenTypes.TIPO){
										iTablaDeSimbolos tablaSimbolo = SingleTabla.getInstance();
										tablaSimbolo.setTipoPendiente(Tipo.parseTipo(this.t.getStringLex()));
										declFunc = false;
										pasoParamCierre = false;
									}
									this.tokenAnt = t;
									t = this.la.getToken();
								}
							...
							case REDUCE:
								...
								try{
									...
									UI.error(err.getTitle(),err.getMessage());
								}catch(CodeException e){
									this.tratarErroresCodigo(e.getTitle()+". "+e.getMessage());
	...
	private void tratarErroresSemanticos(Token t, String error){
		iTablaDeSimbolos tab = SingleTabla.getInstance();
		tab.setErrorMode();
	...
	/** Trata los Errores Semanticos
	 * @param */
	private void tratarErroresCodigo(String error){
		iTablaDeSimbolos tab = SingleTabla.getInstance();
		tab.setErrorMode();
		UI.error("Error en la Generacion de Codigo", error);
		//System.exit(1);
	}
