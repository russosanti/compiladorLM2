...
public class LexicAnalyzer {
	...
	public LexicAnalyzer(File f){
		...
		log.log_normal("Inicializacion de la Compilacion...");
	...
	public LexicAnalyzer(String path){
		try{
			...
			fr = new FileReader(file);
			log = SystemLog.getInstance(path);
			log.log_normal("Inicializacion de la Compilacion...");
			log.log_normal("Comienza el Analizador Lexico");
		}catch (Exception e){
			UI.error("File Exception",e.getMessage());
			log.log_error("File Exception",e.getMessage());
		}
	} 
	/**   
	...
	public Token getToken(){
		...
		if(t.getType() == TokenTypes.ERROR){
			...
			if(t.getType() == TokenTypes.EOF){
				this.log.log_normal(t.toString());
