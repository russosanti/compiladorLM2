	...
	public void generateCode() throws CodeException {
		...
		for(int i=1;i<=cantTemporales;i++){
			...
			if(tabla.isGlobalContext()){
				cg.insertCode("MOV BX, temp"+i);
			}else{
				cg.insertCode("MOV BX, temp"+i+tabla.getContexto());
			}
			cg.insertCode("push BX");
		}
		...
		for(int i=cantTemporales;i>0;i--){
			...
			if(tabla.isGlobalContext()){
				cg.insertCode("MOV temp"+i+", BX");
			}else{
				cg.insertCode("MOV temp"+i+tabla.getContexto()+", BX");
			}
		}
